...
class OutputGenerator {
  formatOutput(summary, format) {
    switch (format.toLowerCase()) {
      ...
      case 'markdown':
      default:
        return typeof summary === 'string' ? summary : this.formatAsMarkdown(summary);
    }
  }
...
## Files Modified
${topFiles.join(', ')}${stats.totalFiles > 5 ? ` (and ${stats.totalFiles - 5} more)` : ''}`;
        break;

      case 'bullets':
        content = `${topCommits.map(c => `• ${c.message}`).join('\n')}
- Files modified: ${topFiles.join(', ')}
- Statistics: ${commits.length} commits | ${stats.totalFiles} files | +${stats.totalInsertions}/-${stats.totalDeletions} lines`;
        break;
      ...
      default:
        ...
        commits.forEach((commit, index) => {
          ...
          if (commit.files && commit.files.length > 0) {
            const mainFiles = commit.files.slice(0, 3).map(f => f.name).join(', ');
  ...
  getCommitType(message) {
    ...
    if (msg.includes('feat') || msg.includes('feature') || msg.includes('add') || 
      ...
      return 'Issue fix';
               ...
               msg.includes('controller') || msg.includes('service')) {
