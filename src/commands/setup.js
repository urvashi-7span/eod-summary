...
async function setupCommand() {
  ...
  console.log(
    chalk.gray(`- AI Provider: ${currentConfig.aiProvider || "Not set"}`)
  );
  console.log(
    chalk.gray(`- Summary Type: ${currentConfig.defaultSummaryType}`)
  );
  console.log(
    chalk.gray(`- Output Format: ${currentConfig.defaultOutputFormat}`)
  );
  console.log();
  ...
  try {
    ...
    let updatedConfig = {
      ...currentConfig,
      aiProvider: providerChoice.aiProvider,
    };

    if (providerChoice.aiProvider === "gemini") {
      console.log(chalk.blue("\n📋 To get your Gemini API key:"));
      console.log(
        chalk.gray("1. Visit: https://makersuite.google.com/app/apikey")
      );
      ...
      const geminiConfig = await inquirer.prompt([
        ...
        },
        {
          ...
          choices: [
            {
              name: "gemini-2.0-flash-exp - Latest experimental model (recommended)",
              value: "gemini-2.0-flash-exp",
            },
            {
              name: "gemini-1.5-flash-002 - Stable legacy model",
              value: "gemini-1.5-flash-002",
            },
            {
              name: "gemini-1.5-pro-002 - More capable legacy model",
    ...
    const preferences = await inquirer.prompt([
      {
        ...
        choices: [
          {
            name: "Quick - Brief summary with key points (recommended)",
            value: "quick",
          },
          {
            name: "Detailed - Comprehensive report for stakeholders",
            value: "detailed",
          },
      ...
      },
      {
        ...
        choices: [
          {
            name: "Markdown - Rich formatting (recommended)",
            value: "markdown",
          },
    ...
    console.log(
      chalk.gray(
        "  eod-summary                    # Generate summary for today"
      )
    );
    console.log(
      chalk.gray("  eod-summary -d 2024-12-15     # Generate for specific date")
    );
    console.log(
      chalk.gray("  eod-summary -t detailed       # Generate detailed summary")
    );
