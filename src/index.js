...
program
  ...
  .option('-d, --date <date>', 'Date for summary (YYYY-MM-DD)', formatDate(new Date()))
  .option('-t, --type <type>', 'Summary type: quick, detailed, bullets', 'quick')
...
program
  ...
  .option('--no-ai', 'Generate summary without AI (template-based)')
  .action(async (options) => {
    if (process.argv.length <= 2 || !['generate', 'setup', 'config'].includes(process.argv[2])) {
      try {
        await generateSummary(options);
      } catch (error) {
        console.error(chalk.red('Error:'), error.message);
...
async function generateSummary(options) {
  ...
  try {
    ...
    const commits = await gitParser.getCommitsByDate(date);
    ...
    if (commits.length === 0) {
      ...
      console.log(chalk.gray('- Check if your Git user email is configured: git config user.email'));
      return;
    }
    ...
    if (options.ai === false) {
      ...
      summary = outputGenerator.genera
      teTemplateBasedSummary(commits, summaryType);
    } else {
      const config = ConfigManager.loadConfig();
      
      if (config.aiProvider === 'gemini' && !config.geminiApiKey) {
        spinner.fail(chalk.red('Gemini API key not configured. Run: eod-summary setup'));
        return;
      }
      
      if (config.aiProvider === 'ollama') {
        const ollamaStatus = await AIProcessor.testOllamaConnection(config.ollamaUrl, config.ollamaModel);
        if (!ollamaStatus.available) {
          spinner.warn(chalk.yellow('Ollama not available, falling back to template...'));
          const outputGenerator = new OutputGenerator();
          summary = outputGenerator.generateTemplateBasedSummary(commits, summaryType);
        } else if (!ollamaStatus.hasModel) {
          spinner.fail(chalk.red(`Model '${config.ollamaModel}' not found. Install it with: ollama pull ${config.ollamaModel}`));
          return;
        }
      }
    ...
    const outputGenerator = new OutputGenerator();
    const formattedOutput = outputGenerator.formatOutput(summary, options.format);
...
function validateSummaryType(type) {
  const validTypes = ['quick', 'detailed', 'bullets'];
  if (!validTypes.includes(type)) {
    throw new Error(`Invalid summary type: ${type}. Valid types: ${validTypes.join(', ')}`);
  }
  return type;
}
